...
public class EndunlockCommand implements CommandExecutor {

    private Main main;

    public EndunlockCommand(Main main) {
        this.main = main;
    }
...
//            String flows = "de290521-fdcf-4829-b72e-9e9abe81b77b";
            ...
            if (uuid.toString().equals(flows) || uuid.toString().equals(flowsalt)) {
                ...
                if (endUnlocked) {
                    player.sendMessage(ChatColor.RED + "the end is already unlocked!");
                    return true;
                }

                main.getConfig().set("end-unlocked", true);
                main.saveConfig();
                player.sendMessage(ChatColor.RED + "unlocked the end");

                for (Player target : Bukkit.getOnlinePlayers()) {
                    target.sendMessage(ChatColor.YELLOW + "End dimension is unlocked!");
                }
